...
export function DialogProvider() {
  const { confirmState, alertState, resolveConfirm, closeAlert } = useDialogStore();
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200/60 p-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex gap-4">
              <div className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
                ...
                confirmState.severity === 'success' ? 'bg-green-100 text-green-600' :
                'bg-blue-100 text-blue-600'
              }`}>
                {confirmState.severity === 'danger' ? <AlertTriangle size={24} /> :
                 confirmState.severity === 'success' ? <CheckCircle size={24} /> :
                 <Info size={24} />}
              </div>
              ...
              <div className="flex-1">
                <h3 className="text-lg font-bold text-slate-900 mb-2">
                  {confirmState.title || (
                    confirmState.severity === 'danger' ? 'Konfirmasi' : 'Informasi'
                  )}
                </h3>
                ...
                <div className="flex justify-end gap-3">
                  ...
                  >
                    {confirmState.severity === 'danger' ? 'Ya, Hapus' : 'Ya, Lanjutkan'}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed bottom-6 right-6 z-9999 animate-in slide-in-from-bottom-5 fade-in duration-300">
          <div className="bg-white rounded-xl shadow-lg shadow-slate-200/50 border border-slate-200/60 p-4 min-w-75 max-w-md flex items-start gap-3 relative">
            ...
            <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
              alertState.severity === 'danger' ? 'bg-red-100 text-red-600' :
              alertState.severity === 'success' ? 'bg-green-100 text-green-600' :
              'bg-blue-100 text-blue-600'
            }`}>
              {alertState.severity === 'danger' ? <AlertTriangle size={18} /> :
               alertState.severity === 'success' ? <CheckCircle size={18} /> :
               <Info size={18} />}
            </div>
            ...
            <div className="flex-1 pr-6">
              <h4 className="text-sm font-bold text-slate-900 mb-1">
                {alertState.title || (
                  alertState.severity === 'danger' ? 'Terjadi Kesalahan' :
                  alertState.severity === 'success' ? 'Berhasil' :
                  'Informasi'
                )}
              </h4>
