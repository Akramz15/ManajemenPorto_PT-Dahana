...
function CustomTooltip({
  ...
  return (
    <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-5 text-sm min-w-56 z-50">
      <p className="font-bold text-slate-800 mb-4 pb-3 border-b border-slate-100 uppercase tracking-widest text-[11px]">
        {label}
      </p>
      <div className="space-y-3">
        <div className="flex justify-between items-center gap-6">
          ...
          <span className="font-bold text-slate-700">
            {rencana?.value != null ? `${rencana.value}%` : "-"}
          </span>
        </div>
        <div className="flex justify-between items-center gap-6">
          ...
          <span className="font-black text-slate-900 text-base">
            {realisasi?.value != null ? `${realisasi.value}%` : "-"}
          </span>
        </div>
        {deviasi && (
          <div className="flex justify-between items-center gap-6 pt-3 mt-3 border-t border-slate-100/80">
            ...
            <span
              className={`font-bold px-2 py-0.5 rounded-md text-xs ${parseFloat(deviasi) >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
            >
              {parseFloat(deviasi) > 0 ? "+" : ""}
              {parseFloat(deviasi)}%
            </span>
          </div>
        )}
    ...
    return (
      <div className="card flex items-center justify-center min-h-90 text-slate-400">
        <p className="text-sm font-medium">
          Data progres (Kurva S) belum tersedia
        </p>
      </div>
    );
  }
