...
function RKAPTooltip({
  ...
  const deviasi =
    realisasi?.value != null && rkap?.value != null
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center gap-6">
          ...
          <span className="font-black text-white">
            {formatRupiah(realisasi?.value ?? 0)}
          </span>
        </div>
        ...
        {deviasi && (
          <div className="flex justify-between items-center gap-6 pt-2 mt-2 border-t border-slate-700/50">
            ...
            <span
              className={`font-bold text-xs ${deviasi >= 0 ? "text-emerald-400" : "text-rose-400"}`}
            >
              {deviasi > 0 ? "+" : ""}
              {formatRupiah(deviasi)}
            </span>
          </div>
        )}
      </div>
...
export function RKAPChart({
  data,
  title = "Target RKAP vs Realisasi (Terkini)",
  headerAction,
}: {
  data: RKAPPoint[];
  title?: string;
      ...
      <div className="w-full flex-1 relative flex flex-col min-h-0">
        <div className="px-1 flex flex-col mb-4">
          ...
          <div className="flex items-baseline gap-3">
            ...
            {devStatus !== 0 && (
              <span
                className={`text-sm font-bold flex items-center ${devStatus > 0 ? "text-emerald-500" : "text-rose-500"}`}
              >
        ...
        <div className="w-full flex-1 min-h-0 mt-2">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip
                content={<RKAPTooltip />}
                cursor={{ stroke: "#f1f5f9", strokeWidth: 40, opacity: 0.5 }}
              />
              <Legend
                wrapperStyle={{
                  fontSize: 12,
                ...
                formatter={(value) => {
                  if (value === "rkap")
                    return (
                      <span className="text-slate-400 ml-1 mr-4">
                        Target (RKAP)
                      </span>
                    );
                  return (
                    <span className="text-emerald-600 ml-1">Realisasi</span>
                  );
                }}
              />
              <Area
