      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center gap-6">
          ...
          <span className="font-bold text-white">
            {rencana?.value != null
              ? rencana.value.toLocaleString("id-ID")
              : "-"}
          </span>
        </div>
        <div className="flex justify-between items-center gap-6">
          ...
          <span className="font-black text-white">
            {realisasi?.value != null
              ? realisasi.value.toLocaleString("id-ID")
              : "-"}
          </span>
        </div>
        {deviasi && (
          <div className="flex justify-between items-center gap-6 pt-2 mt-2 border-t border-slate-700/50">
            ...
            <span
              className={`font-bold text-xs ${parseFloat(deviasi) >= 0 ? "text-emerald-400" : "text-rose-400"}`}
            >
              {parseFloat(deviasi) > 0 ? "+" : ""}
              {parseFloat(deviasi).toLocaleString("id-ID")}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="px-6 flex flex-col mb-4">
        ...
        <div className="flex items-baseline gap-3">
          ...
          {devStatus !== 0 && (
            ...
            >
              {devStatus > 0 ? "▲" : "▼"}{" "}
              {Math.abs(devStatus).toLocaleString("id-ID")}
            </span>
          )}
        </div>
      </div>
