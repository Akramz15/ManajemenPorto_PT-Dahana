...
export function ChartShell({ loading, error, hasData, onUploadClick, children, height = 360 }: ChartShellProps) {
  if (loading) {
    return (
      <div className="card w-full flex items-center justify-center" style={{ minHeight: height }}>
  ...
  if (error || !hasData) {
    return (
      <div className="card w-full flex flex-col items-center justify-center gap-3 text-slate-400" style={{ minHeight: height }}>
        {error ? (
          <>
