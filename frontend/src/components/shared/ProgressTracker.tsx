...
> = {
    ...
    label: "Selesai",
    badgeColor:
      "bg-positive-50 text-positive-700 border-positive-200 hover:bg-positive-100",
  ...
  blocked: {
    label: "Terhambat",
    badgeColor:
      "bg-negative-50 text-negative-700 border-negative-200 hover:bg-negative-100",
    iconColor: "text-white",
...
export function ProgressTracker({
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-base font-bold text-slate-800">
            Timeline Progres & Tim
          </h3>
          <p className="text-xs text-slate-500 mt-0.5">
            Pantau status pencapaian tugas proyek
          </p>
        </div>
      ...
      <div
        className={`relative flex-1 mt-2 ${tasks.length > 0 || isAdding ? "pl-10" : ""}`}
      >
        {/* Vertical Line */}
          ...
          {!tasks.length && !isAdding && (
            <div className="py-24 min-h-87.5 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center bg-slate-50/50">
              ...
              <p className="text-xs text-slate-500 max-w-xs mb-4">
                Mulai kelola progres proyek dengan menambahkan tugas pertama.
              </p>
              {onAddTask && (
                <button
                  onClick={() => setIsAdding(true)}
                  className="flex items-center gap-1.5 px-4 py-2 bg-white border border-slate-200 hover:border-slate-300 shadow-sm text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 transition-all"
                >
          ...
          {tasks.map((task) => {
            ...
            return (
              <div key={task.id} className="relative group">
                {/* Timeline Icon */}
                <div
                  className={`absolute -left-10 top-2 w-6 h-6 rounded-full border-4 border-white ${config.iconBg} ${config.iconColor} flex items-center justify-center z-10 ring-1 ring-slate-200/60 shadow-sm transition-transform group-hover:scale-110`}
                >
                ...
                <div
                  className={`p-4 rounded-xl border transition-all duration-200 ${
                    task.status === "in_progress"
                      ? "bg-primary-50/20 border-primary-100 shadow-[0_2px_10px_-4px_rgba(59,130,246,0.1)]"
                      : task.status === "done"
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <h4
                        className={`text-sm font-semibold truncate ${task.status === "done" ? "text-slate-500 line-through" : "text-slate-800"}`}
                      >
                        {task.title}
                      </h4>
