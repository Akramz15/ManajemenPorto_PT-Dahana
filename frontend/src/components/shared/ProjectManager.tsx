  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingProject) {
        ...
        const { data } = await supabase.from("projects").insert([{ ...payload, created_by: currentUserId }]).select();
  ...
  const handleMoveToBerjalan = async (id: string) => {
    ...
    fileInput.onchange = async (e) => {
      ...
      try {
        ...
        await supabase.from("projects").update({ kategori: "berjalan" }).eq("id", id);
      ...
      <div className="flex flex-col space-y-8">
        ...
        <div className="bg-slate-50/70 p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Nama Proyek *</label>
              <input 
                type="text" 
                required
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              >
                {loading ? <Spinner size="sm" /> : (editingProject ? <Edit2 size={18} /> : <Plus size={18} />)}
        ...
        {/* List */}
        <div>
          <h3 className="text-sm font-bold text-slate-700 mb-4 uppercase tracking-wider">Daftar Proyek</h3>
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50/80 border-b border-slate-200">
                <tr>
                  <th className="px-5 py-4 font-bold text-slate-600">Nama Proyek</th>
                  <th className="px-5 py-4 font-bold text-slate-600 w-32 text-center">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {projects.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-5 py-4">
                      <p className="font-bold text-slate-800">{p.nama_proyek}</p>
                      {p.mitra && <p className="text-[11px] font-medium text-slate-500 mt-1 uppercase tracking-wider">Mitra: {p.mitra}</p>}
                    </td>
                    <td className="px-5 py-4">
                ...
                {projects.length === 0 && (
                  <tr>
                    <td colSpan={2} className="px-5 py-12 text-center text-slate-400 font-medium">Belum ada proyek.</td>
                  </tr>
                )}
              </tbody>
            </table>
