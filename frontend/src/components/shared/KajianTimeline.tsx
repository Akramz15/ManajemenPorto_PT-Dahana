...
> = {
  ...
  in_progress: {
    label: "Berjalan",
    dot: "bg-primary-500 shadow-[0_0_0_4px_rgba(59,130,246,0.1)]",
    bg: "bg-primary-50/50",
  },
  done: {
    label: "Selesai",
    dot: "bg-positive-500",
    bg: "bg-white border border-positive-100",
  },
...
export function KajianTimeline({ tasks, onStatusChange }: KajianTimelineProps) {
    ...
    return (
      <div className="card p-6 min-h-75 flex flex-col relative overflow-hidden group justify-center items-center text-slate-400">
        <p className="text-sm font-medium">
          Belum ada data kajian untuk divisi ini.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="card">
      <h3 className="text-sm font-semibold text-slate-800 mb-6 px-1">
        Tahapan Kajian Kolaboratif
      </h3>
          ...
          {tasks.map((task, idx) => {
            ...
            return (
              <div key={task.id} className="relative pl-10 pr-2">
                {/* Dot */}
                <div
                  className={`absolute left-0 top-3 w-1.75 h-1.75 rounded-full z-10 -translate-x-0.75 transition-all duration-300 ${config.dot}`}
                />

                <div
                  className={`p-4 rounded-xl transition-all duration-200 ${task.status === "done" ? "shadow-sm border border-slate-100" : config.bg}`}
                >
